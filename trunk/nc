...
ack_pkt_tcp = TCP(sport = sport, dport = dport, flags = "A", seq = syn_pkt.seq + 1, ack = ack_num)
...
req_tcp_layer = TCP(sport = sport, dport = dport, flags = "PA", seq = syn_pkt.seq + 1, ack = ack_num)

if eth_layer:
    req_pkt = eth_layer/ip_layer/req_tcp_layer/req_data
else:
